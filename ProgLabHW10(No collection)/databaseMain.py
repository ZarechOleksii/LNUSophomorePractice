...
def database_action(sort_param, order, what, one):
    parameters = ['id', 'title', 'rest_name', 'date', 'time', 'duration', 'price', 'user_id', 'amount']
    if one is None:
        to_do = "Select* From events"
        if what is not None:
            to_do += " where "
            for x in parameters:
                to_do += x + " like \'%" + what + "%\'"
        ...
        if sort_param is not None:
            if sort_param in parameters:
                to_do += " order by " + sort_param
                if order is not None:
                    if order.lower() == 'asc' or order.lower() == 'desc':
                        to_do += " " + order
                    else:
                        return {'status': '400', 'error': 'order can be only asc or desc'}
            else:
                return {'status': '400', 'error': 'no such sort parameter'}
    else:
        to_do = "Select* From events where id = \'" + one + "\';"
    cursor.execute(to_do)

    to_return_data = []
    iteration = 0
    for q in cursor:
        to_return_data.append(dict())
        for z in range(0, len(parameters)):
            to_return_data[iteration][parameters[z]] = str(q[z])
        iteration += 1
...
def print_all_and_search():
    ...
    if 'what' in request.args:
        search = request.args['what']
    if 'sort_by' in request.args:
        sorting = request.args['sort_by']
    if 'sort_type' in request.args:
        order = request.args['sort_type']
    to_return = database_action(sorting, order, search, None)
...
def print_one(g_id):
    to_return = database_action(None, None, None, g_id)
...
def deleting_one(g_id):
    ...
    global current_ord
    if database_action(None, None, None, str(g_id)):
